...
/*
    ...
    {
      (float phi, float theta, float r) = AJakPanJezusPowiedział.TakJakPanJezusPowiedzial(wariat.position.x, wariat.position.y, wariat.position.z);

      //List<List<double>> neighbour_coordinates_polar = new List<List<double>>(wariat.neighbours.size());
      List<float> weights = new List<float>[wariat.neighbours.Count()];

      for (int i = 0; i < wariat.neighbours.Count(); i++)
      {
        (float temp_phi, float temp_theta, float temp_r) = AJakPanJezusPowiedzia.TakJakPanJezusPowiedzial(wariat.neighbours[i].position.x, wariat.neighbours[i].position.y, wariat.neighbours[i].position.z);

        delta_theta = Mathf.Abs(theta - delta_theta);
        delta_phi   = Mathf.Abs(phi   - delta_phi  );
        ...
        if      (temp_theta >= theta && temp_phi >= phi)
        {
          weights[i] =  10 * (Mathf.Abs(delta_theta - delta_phi);
        }
        else if (temp_theta <  theta && temp_phi >= phi)
        {
          weights[i] =  1  * (Mathf.Abs(delta_phi - delta_theta);
        }
        else if (temp_theta <  theta && temp_phi <  phi)
        {
          weights[i] = -1  * (Mathf.Abs(delta_theta - delta_phi);
        }
        else                                            
        {
          weights[i] = -10 * (Mathf.Abs(delta_phi - delta_theta);
        }
      }

      wariat.neighbours.sort(weights);
...
*/
    VertexPoint FindClosestInList(VertexPoint point,List<VertexPoint> list,List<VertexPoint> exclude)
        ...
        neighbour_coordinates_polar.Add(new List<double>(new double[3]));
        neighbour_coordinates_polar[i][0] = tempphi
        neighbour_coordinates_polar[i][1] = wariat.neighbours[i].position.y;
        neighbour_coordinates_polar[i][2] = wariat.neighbours[i].position.z;
